...
def setup_mlflow_experiment(params: Dict[str, Any]) -> str:
    ...
    if experiment_id is not None:
        ...
        try:
            ...
            logger.info(
                f"Using existing experiment: ID={experiment_id}, Name={experiment.name}"
            )
    ...
    try:
        ...
        if experiment is not None:
            ...
            logger.info(
                f"Using existing experiment: ID={experiment_id}, Name={experiment_name}"
            )
        else:
            # Create new experiment
            experiment_id = mlflow.create_experiment(experiment_name)
            logger.info(
                f"Created new experiment: ID={experiment_id}, Name={experiment_name}"
            )
...
def separate_features_target(
    ...
    logger.info(
        "Separated features/target: features=%d target=%s",
        len(feature_cols),
        target_column,
    )
...
def train_catboost_model(
    ...
    with mlflow.start_run(run_name="catboost_training"):
        ...
        model.fit(
            X_train, y_train, eval_set=(X_test, y_test), use_best_model=True, plot=False
        )
