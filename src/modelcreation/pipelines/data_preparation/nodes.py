...
def prepare_model_data(
    ...
    if isinstance(forced_passthrough, str):
        forced_passthrough = [
            c.strip() for c in forced_passthrough.split(",") if c.strip()
        ]
    ...
    existing_refs = [
        ...
        if c in raw_data.columns and c not in columns_to_keep
    ]
    if existing_refs:
        logger.info(
            f"Including reference columns (passthrough, not for training): {existing_refs}"
        )
        columns_to_keep.extend(existing_refs)
    if old_model_column and old_model_column in existing_refs:
        logger.info(
            f"Old model column '{old_model_column}' retained for evaluation only"
        )

    # Add forced passthrough columns if they exist in data
    forced_present = [
        c
        for c in forced_passthrough
        if c in raw_data.columns and c not in columns_to_keep
    ]
    if forced_present:
        logger.info(
            f"Including forced passthrough columns (always kept, not for training): {forced_present}"
        )
        columns_to_keep.extend(forced_present)
    missing_refs = [c for c in reference_columns if c not in raw_data.columns]
    if missing_refs:
        logger.warning(
            f"Reference columns not found and will be skipped: {missing_refs}"
        )
    ...
    for col in feature_columns:
        logger.info(
            f"  {col}: mean={prepared_data[col].mean():.2f}, "
    ...
    logger.info(
        f"  {target_column}: mean={prepared_data[target_column].mean():.2f}, "
